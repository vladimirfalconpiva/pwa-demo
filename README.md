...
Then go to front directory, run command to kill current process on port:3000 and run development process.
...
Base props used by all content elements:
...
📍 ```this.data.content``` contains all custom data delivered with content element by API. Fields from this object are accessible directly in your components ( you have to define them as a props)
...
edit `nuxt.config.js` ( if you did not define it before )
